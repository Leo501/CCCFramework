...
KBEngine.Entity = KBEngine.Class.extend({
	...
	baseCall: function () {
		...
		if (this.base == undefined) {
			...
			KBEngine.Event.fire('baseCall_is_null');
			return;
		}
	...
	cellCall: function () {
		...
		if (this.cell == undefined) {
			...
			KBEngine.Event.fire('cellCall_is_null');
			return;
		}
...
// 		console.log("sbsbsbsb22333333455554")
//         var result = '';
//         for (var i = 0; i < 4; ++i) {
...
// 		console.log("cmsmdsmdsmdmmdsds", s)
...
//     this.parseDefaultValStr = function (v) {
// 		console.log("sbsbsbsb22333")
//         return JSON.stringify(v);
//     }

//     this.isSameType = function (v) {
// 		console.log("sbsbsbsb2233333344")
...
KBEngine.KBEngineApp = function (kbengineArgs) {
	...
	this.reset = function () {
		...
		KBEngine.app.clientVersion = '1.3.3';
...
KBEngine.create = function (args) {
	...
	KBEngine.app.Client_onImportClientMessages = function (msg) {
		var packages = splicePackage['Client_onImportClientMessages'];
		if (packages) {
			var newData = mergeArrayBuffers(packages.data, msg.data);
			if (newData.byteLength >= packages.len) {
				msg.data = newData;
				delete splicePackage['Client_onImportClientMessages'];
				return Client_onImportClientMessages.call(this, msg);
			} else {
				packages.data = newData;
				return;
			}
		}
		var stream = new KBEngine.MemoryStream(msg.data);
	...
	KBEngine.app.onmessage = function (msg) {
		var packages = splicePackage['onmessage'];
		if (packages) {
			msg.data = mergeArrayBuffers(packages.data, msg.data);
		}
		var stream = new KBEngine.MemoryStream(msg.data);
		if (packages) {
			stream.rpos = packages.rpos;
		}
		stream.wpos = msg.data.byteLength;
