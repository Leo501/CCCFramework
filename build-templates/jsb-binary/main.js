(function () {
    // 判断安装包版本是否比本地缓存的新，如果新则要清一下本地缓存
    var version = "1.0.0";
    let curVersion = cc.sys.localStorage.getItem("CurrentVersion_Bobby") || "1.0.0";
    var storagePath = ((jsb.fileUtils ? jsb.fileUtils.getWritablePath() : '/') + 'Bobby');
    ...
    if (versionCompareHandle(version, curVersion) >= 0) {
        ...
        cc.sys.localStorage.setItem('CurrentVersion_Bobby', version);
        curVersion = version;
    }
